...
shelltools.export("GOROOT", "%s/go-go1.10" % get.workDIR())
...
def build():
    ...
    shelltools.export("GOPATH", "%s" % get.workDIR())
    shelltools.export("GOROOT_FINAL", "/usr/lib/go")
    shelltools.export("GOROOT_BOOTSTRAP", "%s/go-go1.10/go-linux-amd64-bootstrap" % get.workDIR())

    shelltools.export("GOOS","linux")
    shelltools.export("GOARCH","amd64")
    shelltools.system("bash make.bash")

    shelltools.cd("%s/go-go1.10" % get.workDIR())
